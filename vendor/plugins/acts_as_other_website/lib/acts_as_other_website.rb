...
module ActsAsOtherWebsite
  module ActionControllerMethods
    module InstanceMethods
      def on_invalid_credentials
        ...
        redirect_to new_session_path(:username => params[:session] ? params[:session][:username] : nil)
      end
    ...
    def acts_as_other_website(options = {})
      ...
      attr_reader :mechanized_session
      ...
      rescue_from Mechanize::ResponseCodeError do |ex|
  ...
  module MechanizedSessionMethods
    def signed_out?(page)
      # override with application specific logic to determine if page is not as expected
      false
    end

    def check_for_signed_out!(page)
      if signed_out?(page)
        raise MechanizedSession::InvalidSession
      end
    end
...
MechanizedSession.send(:include, ActsAsOtherWebsite::MechanizedSessionMethods)